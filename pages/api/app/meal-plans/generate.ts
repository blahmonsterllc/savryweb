...
export default async function handler(
  ...
  try {
    ...
    // Discounts/scraping disabled for now
    const discounts: any[] = []
    
    if (params.budget && userTier === 'FREE') {
    ...
    for (const day of generatedMealPlan.mealPlan.days) {
      for (const [mealType, mealData] of Object.entries(day.meals)) {
        const typedMeal = mealData as any

        const recipe = await prisma.recipe.create({
          data: {
            userId,
            name: typedMeal.name,
            ingredients: typedMeal.ingredients,
            instructions: typedMeal.instructions,
            servings: 2,
            autoGenerated: true,
            source: 'GENERATED',
          }
